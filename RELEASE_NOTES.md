...
|---|---|---|
| **2.5.2 (native HTML tree)** *(testing)* | 2026-06-15 | CopyTrust and Drop Verify: recursive HTML tree output now uses native file enumeration and no longer requires the external `tree` command. One HTML per top-level folder and Entire project preserve the existing collapsible HTML output without waiting on `tree -J` child processes. |
