...
**Stable: CopyTrust 2.5.3 / Drop Verify 2.5.3** — sorted-copy MHL verification fix, per-copy provenance records, output-toggle fixes, and built-in HTML tree/index enumeration. **MHL Verify and mhl-tool stable at 2.5.1** — ASC MHL v2.0 (Silverstack 9+) support and the Verify action. **CopyTrust 2.5.4 and Drop Verify 2.5.4 are in testing (Build 3)** — new **Split large contact sheets** setting (numbered part PDFs for large cards), a fixed large-card contact sheet timeout that no longer leaves orphaned PDFs, Finder reveal for multi-part auto-open, explicit skip logging for disabled artifacts, generation heartbeat logs, neutral `tree_…` filenames, a copy heartbeat in the live activity log, and an orange menu bar verification bar with its own percentage. One-line history in [RELEASE_NOTES.md](RELEASE_NOTES.md); detailed changes in each app's docs.
